...
function validateFileForFormat(file, format) {
	...
	if (!cfg) return { ok: false, message: `Unsupported format: ${format}` };

	// Prefer extension check (file.type can be empty or unreliable in some browsers)
	const extOk = cfg.exts.includes(ext);
	const mimeOk = cfg.mimes.length === 0 ? true : cfg.mimes.includes(mime);
	...
	if (size > cfg.maxBytes) {
		return {
			ok: false,
			message: `File too large (${bytesToMB(size)} MB). Max ${bytesToMB(
				cfg.maxBytes,
			)} MB allowed.`,
		};
	}
