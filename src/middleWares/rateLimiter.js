const MAX_ATTEMPTS_STAGE_1 = 5; // 5 attempts, cooldown 15 mins
const MAX_ATTEMPTS_STAGE_2 = 3; // 3 attempts, cooldown 1 hour
const MAX_ATTEMPTS_STAGE_3 = 3; // 3 attempts, cooldown 1 day
...
export const incrementLoginAttempts = async (user) => {
	...
	if (user.lockUntil && user.lockUntil < now) {
		updates = { loginAttempts: 1, lockUntil: null }; // Reset login attempts
	}
	...
	return user.updateOne(updates); // Apply the updates to the user
};
